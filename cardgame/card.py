...
from .logic import *
from .logic.actions import Destroy, Deaths, Attack, AuraBuff
from .logging import *
#import logging
...
class BaseCard(Entity):
	...
	def play_targets(self):
		...
		if self.get_all_actions("corrupt"):
			target_selectors = [ALLIED_UNITS] + target_selectors
		targets = []
	...
	def refresh_buff(self, source, buff_id, **kwargs):
		for buff in self.buffs:
			...
			action_log.log("Aura from %r buffs %r with %r", source, self, buff_id)
	...
	def refresh_tags(self, source, tags):
		for slot in self.slots:
			...
			action_log.log("Creating %r", buff)
	...
	def discard(self):
		action_log.log("%s discards %r", self.controller, self)
	...
	def draw(self):
			...
			#self.discard()
		#else:
		action_log.log("%s draws %r", self.controller, self)
		self.zone = Zone.HAND
...
class Unit(LiveEntity):
	...
	def _set_zone(self, value):
		...
		if self.zone == Zone.PLAY:
			action_log.log("%r is removed from the field", self)
	...
	def _set_zone(self, zone):
		if zone == Zone.PLAY:
			...
			if self.zone == zone:
				...
				action_log.logger.warning("Trying to remove %r which is already gone", self)
				return
